...
$slider->addAction(new SWFAction("
	if (dragging) {
		...
		this._x = $sizeprompt * (_root._currentframe - 1) / $len;
	}
...
if (isset($_GET['nostart'])) {
	...
	// prevent looping even if the movie has autostart set
	$m->add(new SWFAction("
					if(_root.started)
					_root.stop();")
	);
