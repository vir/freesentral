...
//	print 'In order to activate the Auto Attendant you need to add a did(direct inward dialing) to it.';
	...
	{	
		...
		tableOfObjects($dids, $formats, "did", array("&module=dids&method=edit_did"=>'<img src="images/edit.gif" title="Edit" alt="edit"/>', "&module=dids&method=delete_did"=>'<img src="images/delete.gif" title="Delete" alt="delete"/>'),array("&module=dids&method=add_did"=>"Add DID"));
	}else{
...
function keys($wizard = false)
{
	...
	tableOfObjects($keys, array("status", "key","destination","description"), "key for auto attendant", array("&method=edit_key"=>'<img src="images/edit.gif" title="Edit" alt="edit"/>', "&method=delete_key"=>'<img src="images/delete.gif" title="Delete" alt="delete"/>'), array("&method=add_key"=>"Add key"));
}
...
function prompts($wizard=false)
{
	...
	tableOfObjects($prompts, array("status", "prompt", "function_getfilesize:size"=>"prompt"), "prompt",array("&method=edit_prompt"=>'<img src="images/edit.gif" alt="edit" title="Edit"/>', "&method=listen_prompt"=>'<img src="images/listen.gif" alt="play" title="Listen"/>', "&method=reupload_prompt"=>'<img src="images/upload.gif" alt="upload" title="Upload"/>'));
}
...
function upload_prompts_database()
{
	...
	if(strtolower(substr($online,-4)) != ".mp3")
	{
		wizard("File format must be .mp3");
		return;
	}

	if(strtolower(substr($offline,-4)) != ".mp3")
	{
		wizard("File format must be .mp3");
		return;
	}
